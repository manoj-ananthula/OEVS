...
class voting_system:
    def __init__(self):
        self.db=connector.connect(host='127.0.0.1',port = 3306,user='root',password='MySQL', database='voting_system')
    ...
    def login(self):
        while True:
            ...
            Password=input("Password: ")
            query="select Def_Password from user_table where VoterId='{}' and Aadhaar='{}'".format(VoterId,Aadhaar)
            cur=self.db.cursor()
            cur.execute(query)          
            for p in cur:
                if Password==p[0]:
    ...
    def after_login(self,Aadhaar):
        while True:
            query="select PartyLeader from party_table where Aadhaar='{}'".format(Aadhaar)
            cur=self.db.cursor()
            cur.execute(query)
